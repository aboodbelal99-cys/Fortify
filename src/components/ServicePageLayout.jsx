...
export default function ServicePageLayout({ service }) {
  ...
  const problem = isAr ? service.problemAr : service.problem;
  const solution = isAr ? service.solutionAr : service.solution;
  const features = isAr ? service.featuresAr : service.features;
  const industries = isAr ? service.targetIndustriesAr : service.targetIndustries;
  const painPoints = t('servicePage.painPoints');
  const solutionBenefits = t('servicePage.solutionBenefits');
    ...
    <>
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex items-center justify-center gap-2 text-sm text-text-muted mb-8">
            <Link href="/" className="hover:text-gold transition-colors">{t('servicePage.home')}</Link>
            <span>/</span>
            <Link href="/#services" className="hover:text-gold transition-colors">{t('servicePage.services')}</Link>
            <span>/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6">
                {t('servicePage.challengeTitle')} <span className="text-red-400">{t('servicePage.challengeHighlight')}</span>
              </h2>
            ...
            <div className="glass-card p-8 relative overflow-hidden">
              ...
              <div className="space-y-4">
                {Array.isArray(painPoints) && painPoints.map((pain, i) => (
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="order-2 lg:order-1 glass-card p-8 relative overflow-hidden">
              ...
              <div className="space-y-4">
                {Array.isArray(solutionBenefits) && solutionBenefits.map((benefit, i) => (
                  <div key={i} className="flex items-start gap-3">
            ...
            <div className="order-1 lg:order-2">
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6">
                ...
                <span className="font-bold">
                  Fort<span className="text-gold">ify</span>
                </span>{' '}
                <span className="gradient-text">{t('servicePage.solutionHighlight')}</span>
              </h2>
              <p className="text-text-secondary leading-relaxed text-lg">
                {solution}
              </p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">
              {t('servicePage.featuresTitle')} <span className="gradient-text">{t('servicePage.featuresHighlight')}</span>
            </h2>
            <p className="text-text-secondary max-w-xl mx-auto">
              {t('servicePage.featuresDescription')}
            </p>
          </div>
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">
              {t('servicePage.industriesTitle')} <span className="gradient-text">{t('servicePage.industriesHighlight')}</span>
            </h2>
            <p className="text-text-secondary max-w-xl mx-auto">
              {t('servicePage.industriesDescription')}
            </p>
          </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-6">
            {t('servicePage.ctaTitle')} <span className="gradient-text">{title}</span>?
          </h2>
